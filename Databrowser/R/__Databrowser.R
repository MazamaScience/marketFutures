...
library(RJSONIO) # for JSON support
...
source("__DATABROWSER_PATH__/R/MarketFuturesFunctions.R")
source("__DATABROWSER_PATH__/R/futuresChainPlot.R")
...
__DATABROWSER__ <- function(jsonArgs='{}') {
  ...
  if (infoList$plotType == 'Futures') {
    ...
    returnValues <- futuresChainPlot(dataList,infoList,textList)
